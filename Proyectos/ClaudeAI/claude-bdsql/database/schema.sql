...
-- Version: 2.0 - CON SISTEMA DE CLONES Y SOFT DELETE
-- Base: MySQL 8.0+
-- Fecha: 2025-10-28
-- 
-- CAMBIOS PRINCIPALES:
-- - Sistema de propiedades físicas + promociones (clones)
-- - Soft delete (nunca se borra, solo se inactiva)
-- - Tracking completo de clones activos/pausados/inactivos
-- - Historial de precios con eventos (pausa/reactivación)
-- - Multimedia con retención configurable (365 días default)
-- - Textos versionados (actual + anterior)
-- =========================================
...
-- Sin cambios mayores - igual que antes
-- =========================================
CREATE TABLE zonas (
...
CREATE TABLE inmuebles (
    ...
    precio_estim_constr DECIMAL(15,2),
    ...
    calif_terreno_brk JSON,
    calif_construccion_brk JSON,
    calif_entorno_brk JSON,
    ...
    status_promocion ENUM('promo_activa', 'promo_pausada', 'promo_concluida') DEFAULT 'promo_activa',
    ...
    INDEX idx_status_promocion (status_promocion),
...
-- =========================================
-- TABLA 11: CATALOGOS_OPCIONES (Opciones predefinidas para campos múltiples)
-- =========================================
CREATE TABLE catalogos_opciones (
    id_catalogo INT PRIMARY KEY AUTO_INCREMENT,
    nombre_campo VARCHAR(100) NOT NULL,
    valor_opcion VARCHAR(200) NOT NULL,
    orden INT DEFAULT 0,
...
COMMENT='Catalogo de opciones predefinidas para campos de seleccion multiple';
...
-- =========================================
-- TRIGGER: Calcular edad al insertar inmueble
-- =========================================
DELIMITER //

CREATE TRIGGER trg_inmuebles_calcular_edad_insert
BEFORE INSERT ON inmuebles
FOR EACH ROW
BEGIN
    IF NEW.ano_construccion IS NOT NULL OR NEW.ano_ultima_remodelacion IS NOT NULL THEN
        SET NEW.edad = YEAR(CURRENT_DATE) - GREATEST(
            COALESCE(NEW.ano_construccion, 0),
            COALESCE(NEW.ano_ultima_remodelacion, 0)
        );
    END IF;
END//
...
-- =========================================
-- TRIGGER: Calcular edad al actualizar inmueble
-- =========================================
DELIMITER //

CREATE TRIGGER trg_inmuebles_calcular_edad_update
...
WHERE i.status_promocion IN ('promo_activa');
...
WHERE i.status_promocion = 'promo_activa'
